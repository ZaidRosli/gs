...
int skip_separator(char c, FILE *fp) {
	fprintf(stderr, "DEBUG: Skipping separator: '%c', Line: %d\n", c, linenum); 
    ...
    switch (c) {
        ...
        case '\r': // Carriage return
            cbuf = (char) fgetc(fp);
            if (cbuf == '\n') {
                cbuf = (char) fgetc(fp); // Read next character if it's a line feed
            }
        ...
        case '/': // Start of multi-line comment
            ...
            error("Unexpected token after '/'."); // Handle unexpected character
...
int skip_comment(FILE *fp, int sep_type) {
    int start_line = linenum; // Declare start_line to track where the comment begins

    // Process characters in the comment
    while ((cbuf = (char) fgetc(fp)) != EOF) {
        if (sep_type == 2 && cbuf == '}') { // End of single-line comment
            cbuf = (char) fgetc(fp); // Move to next character
            return 2; // Comment successfully skipped
        } else if (sep_type == 3 && cbuf == '*') { // Potential end of multi-line comment
            if ((cbuf = (char) fgetc(fp)) == '/') { // Confirm end of multi-line comment
                cbuf = (char) fgetc(fp);
                return 3; // Comment successfully skipped
            }
        }

        // Handle new lines within the comment
        if (cbuf == '\n') {
            linenum++;
        } else if (cbuf == '\r') {
            if ((cbuf = (char) fgetc(fp)) == '\n') { // Handle Windows-style new lines
                linenum++;
            } else {
                ungetc(cbuf, fp); // Revert if not a newline
                linenum++;
            }
        }
    }

    // If EOF is reached before the comment is closed
    printf("This is an unclosed comment started at line %d\n", start_line);
    return 1; // Indicate unclosed comment
}
...
int identify_symbol(char *tokenstr, FILE *fp) {
    fprintf(stderr, "DEBUG: Checking symbol '%s'\n", tokenstr); // Add this
    switch (tokenstr[0]) {
        case '(': return TLPAREN;
        case ')': return TRPAREN;
        case '*': return TSTAR;
...
int scan(FILE *fp) {
	...
	fprintf(stderr, "DEBUG: Current char: '%c', Line: %d\n", cbuf, linenum); // Add this
