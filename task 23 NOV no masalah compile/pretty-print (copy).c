...
// Sanitize string_attr before printing sebab ada masalah unicodeDecodeerror
...
if (token != TRPAREN) {
    fprintf(stderr, "line%d WARNING: Symbol ')' is not found. Skipping to safe point.\n", linenum);
    skip_to_safe_point(fp); // Attempt to recover
    return WARNING; // Continue with parsing
}
...
int parse_expression(FILE *fp) {
    ...
    if (token != TRPAREN) {
        fprintf(stderr, "line%d WARNING: Expected ')', but not found. Skipping.\n", linenum);
        while (token != TSEMI && token != TEND && token != EOF) {
            token = scan(fp);
        }
        return NORMAL;  // Allow recovery
    }
...
int parse_factor(FILE *fp) {
    switch (token) {
        ...
        case TLPAREN:
            ...
            if (token != TRPAREN) {
                fprintf(stderr, "line%d WARNING: Expected ')', skipping.\n", linenum);
                skip_to_safe_point(fp);
                return NORMAL;  // Allow recovery
            }
            printf("%s ", tokenstr[token]);
            token = scan(fp);
...
int report_error(int detected_line, int expected_line) {
    if (abs(detected_line - expected_line) > 1) {
        fprintf(stderr,
                "Warning: Detected error at line %d. Expected around line %d (±1).\n",
                detected_line, expected_line);
        fprintf(stderr, "Suggestion: Check token alignment or comment handling.\n");
